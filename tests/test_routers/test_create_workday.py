...
async def test_create_workday(mock_empty_db_session):
    ...
    assert response.json() == {'action': 'workday_created', 'workday_id': 'None', 'date': '2025-01-12'}
...
async def test_create_workday_already_exists(mock_db_session_with_existing_workday):
    response = client.post("/customers/workday/", json={"date": "30.03.2024", "day_status": "open"})
...
async def test_create_workday_invalid_date_format():
    ...
    assert str(excinfo.value) == "time data '12-01-2025' does not match format '%d.%m.%Y'"
